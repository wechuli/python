...
# out the second word for each From line, then you will also count the number of From (not From:) lines and print out a
# count at the end. This is a good sample output with a few lines removed: python fromcount.py
...
fhand=open(r'E:\Study\Python\python-codes\files\mbox.txt')
count=0
for line in fhand:
    if line.find('From ') is not -1:
        #print(line)
        count=count+1
        line.rstrip()
        words=line.split()
